...
public class GenerateScreenShotUtil {

    public interface OnImageWrite {

        void write(BufferedImage image, long timeStampInMicroSec);
    }

    public static void generateScreenShot(File sourceMediaFile, OnImageWrite callBack) throws FileNotFoundException {
        if (!sourceMediaFile.exists()) {
            throw new FileNotFoundException("Source media file " + sourceMediaFile + " is not found");
        }

        if (callBack == null) {
            throw new IllegalArgumentException("callBack of OnImageWrite can not be null");
        }

        String filename = sourceMediaFile.getAbsolutePath();
        ...
        if (container.open(sourceMediaFile.getAbsolutePath(),
            ...
            if (packet.getStreamIndex() == videoStreamId) {
                ...
                while (offset < packet.getSize()) {
                    ...
                    if (picture.isComplete()) {
                        ...
                        //                            final long millisecondsTolerance = 50; // and we give ourselfs
                        // 50 ms of tolerance
                        ...
                        //                                    // we might get this when the user closes the dialog
                        // box, so
                        ...
                        System.out.println("Got image, timestamp = " + picture.getTimeStamp() + ", " +
                                picture.getFormattedTimeStamp());
                        ...
                        if (picture.getTimeStamp() > nextPeriod) {
                            ...
                            if (converter == null) {
                                converter = ConverterFactory.createConverter(ConverterFactory.XUGGLER_BGR_24, newPic);
                            }
                            BufferedImage javaImage = converter.toImage(newPic);
                            callBack.write(javaImage, picture.getTimeStamp());
